...
function getRandomQuote(){
    ...
    let randomQuote = quotes[getRandom]
    return randomQuote;
}
...
function printQuote(){
    ...
    let QuoteString = `<p class="quote">${quote1.quote}</p><p class="source">${quote1.source}`;
    if(quote1.citation){QuoteString += `<span class="citation">${quote1.citation}</span>`}
    if(quote1.year){QuoteString += `<span class="year">${quote1.year}</span></p>`}
    else{QuoteString += '</p>'};
